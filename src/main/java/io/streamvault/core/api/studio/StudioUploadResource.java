package io.streamvault.core.api.studio;
...
@Tag(name = "Studio — Upload", description = "Upload audio files to your own library (ADMIN or ARTIST)")
public class StudioUploadResource {

    private static final Logger LOG = LoggerFactory.getLogger(StudioUploadResource.class);
    ...
    @Operation(summary = "Upload audio files as the authenticated artist/admin")
    ...
    @APIResponse(responseCode = "422", description = "Unsupported file format")
    public Uni<Response> upload(UploadForm form) {
        LOG.debug("action=studio_upload_received files={}", form.files == null ? 0 : form.files.size());
        if (form.files == null || form.files.isEmpty()) {
            return Uni.createFrom().item(
                    Response.status(400).entity(new ErrorResponse("NO_FILES", "At least one file is required")).build());
        }
    ...
    public Response handleLibraryException(LibraryException e) {
        return switch (e.error) {
            ...
            case LibraryError.StorageError x -> {
                LOG.error("studio upload storage failed: {}", x.message());
